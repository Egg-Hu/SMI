# transform vits to quantizable version which can also stop feeding forward specific patches
...
__all__ = ['deit_tiny_patch16_224','deit_base_patch16_224']
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = (
            ...
            qkv[1], 
            qkv[2]
        )  # make torchscript happy (cannot use tensor as tuple) #(B,heads,N,C//heads)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.num_features = (
            self.embed_dim
        ) = embed_dim  # num_features for consistency with other quantization
    ...
    def forward_features(self, x,current_abs_index=None,next_relative_index=None):
        B = x.shape[0]
        ...
        x = self.qact1(x)
        if current_abs_index!=None and next_relative_index!=None:
            if (current_abs_index.shape[1])!=(next_relative_index.shape[1]):
    ...
    def forward(self, x,current_abs_index=None,next_relative_index=None):
        x = self.forward_features(x,current_abs_index,next_relative_index)
        x = self.head(x)
        x = self.act_out(x)
        return x
